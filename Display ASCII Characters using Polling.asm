...
; THE LAST SUBROUTINE IS CALLED "SDIPLAYED" WHICH DISPLAYAS A MESSAGE DEPENDING ON WHAT BAUD RATE HAS BEEN SELECTED WHICH IS STORED AT 100H AND EVERY TIME TF0 OVERFLOWS A INTERRUPT GETS GENERATED WHICH IS STORED AT 140H WHICH WILL DISPLAY WHAT'S ON THE INPUT PORT P2 INTO THE OUTPUT PORT P0.
; THE MAIN PROGRAM ACTS AS THE DRIVER CODE WHICH CALLS EACH SUBROUTINE IN ORDER.
					...
					CJNE				A, B, CONTIN; COMPARE ASCII CHARACTER IN A TO B WHICH HOLDS END MARKER ASCII CHARACTER FOR MESSAGE IF NOT EQUAL JUMP TO CONTAIN IF EQUAL EXECUTE LINES BELOW
					JMP					FINISH
					
					CONTIN:
					MOV 				SBUF,A ;LOAD THE DATA
					JNB 				TI,$; POLLING MODE WAIT FOR CHARACTER TO BE PROCESSED ONCE TI=1 MEANS OV AND CHARACTER IS DONE BEING PROCESSED IF TI=0 THE CHARACTER IS CURRENLY IN PROCCESS AND WE MUST WAIT UNTIL TI=1 FLAG SET BY HARDWARE
					CLR 				TI; GET READY FOR THE NEXT CHARACTER
					INC 				DPTR; NEXT ONE
					JMP 				BACK; STAY IN LOOP
					
					FINISH:
					RET
					
				
					ORG 				140H; PUT TIMER0 HANDLER AT ADDRESS 140H
					TIMER0H:
